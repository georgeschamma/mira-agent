...
- This app is Phase 1: FastAPI app spine, Supabase Auth/RLS proof, and a thin real analysis workflow.
- `/api/analyze` runs a sequential LangGraph shell: router -> Exa research -> PydanticAI content recommendations.
- Do not add CRM, GA4, full product UI, payments, public signup, Redis, Airflow, OpenSearch, vector RAG, Azure deploy, eval suites, benchmarks, or CRM writeback in this phase.
- Use one service: FastAPI serves API routes and, once built, the React/Vite static bundle.
- Runtime database reads/writes must use the user's Supabase JWT with the anon key. Never use the service-role key on request paths.
